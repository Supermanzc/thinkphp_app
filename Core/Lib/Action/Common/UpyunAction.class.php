...
class UpyunAction extends Action{
	...
	public function upload(){
		...
		if ($this->upload_type=='upyun'){
			...
			$options['return-url'] = C('site_url').'/index.php?g=Common&m=Upyun&a=uploadReturn'; /// 页面跳转型回调地址
			...
			if (IS_POST){
				...
				echo '<script>location.href="/index.php?g=Common&m=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
			}else {
				$this->display('local');
			}
		}
